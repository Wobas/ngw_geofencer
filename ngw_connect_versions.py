...
class LinkParametersHolder:
    ...
    def run_script(self):
        ...
        if ans['status'] == 'ok':
            ...
            if ans['status'] == 'ok':
                ...
                ans = sorted(top_layer_difference['dif_list']+bottom_layer_difference['dif_list'], key=LinkParametersHolder.__get_time)
                print('Объединненый список изменений в обоих слоях:', ans)
                ...
                layers = gpd.list_layers(file_name_and_path_top_layer)
                top_layer_geometry = gpd.read_file(file_name_and_path_top_layer, layer=layers['name'][0])
                print(top_layer_geometry.iterrows())

                file_name_and_path_bottom_layer = os.path.join(layers_path, f'layer_{self.bottom_layer_id}.gpkg')
                layers = gpd.list_layers(file_name_and_path_bottom_layer)
                bottom_layer_geometry = gpd.read_file(file_name_and_path_bottom_layer, layer=layers['name'][0])
    ...
    def __get_layers_gpkg(self):
        ...
        if top_layer_info.status_code == 200 and bottom_layer_info.status_code == 200:            
            ...
            message = f'Request errors when getting GPKG files: {top_layer_info.status_code} AND {bottom_layer_info.status_code}'
        
        elif top_layer_info.status_code != 200:
            message = f'Request error when getting top GPKG file: {top_layer_info.status_code}'
        
        elif bottom_layer_info.status_code != 200:
            message = f'Request error when getting bottom GPKG file: {bottom_layer_info.status_code}'

        top_layer_info.close()
        bottom_layer_info.close()

        return LinkParametersHolder.__handle_error(message)
            ...
            if top_layer_info['status'] == 'ok' and bottom_layer_info['status'] == 'ok':
                ...
                if (last_saved_version_top_layer < latest_version_top_layer['version'] and last_saved_version_bottom_layer < latest_version_bottom_layer['version']):
                    ...
                    if (top_layer_dif_info['status'] == 'ok' and bottom_layer_dif_info['status'] == 'ok'):
                        ...
                        ans = sorted(top_layer_dif_info['dif_list']+bottom_layer_dif_info['dif_list'], key=LinkParametersHolder.__get_time)
                        
                        layers_path = os.path.join(self.tmp_files_path, 'layers')
                        file_name_and_path_top_layer = os.path.join(layers_path, f'layer_{self.top_layer_id}.gpkg')
                        file_name_and_path_bottom_layer = os.path.join(layers_path, f'layer_{self.bottom_layer_id}.gpkg')
                        # gdf = gpd.read_file(file_name_and_path_top_layer)

                        layers = gpd.list_layers(file_name_and_path_top_layer)
                        print("Доступные слои:", layers)
    ...
    def __get_latest_version_and_epoch(self, layer_id):
        ...
        if layer_info.status_code == 200:
            # it would probably be better to check if these keys exist
        ...
        if difference_versions_info.status_code == 200:
            ...
            if result.status_code == 200:
                ...
                if requests_for_versions['status'] == 'ok':
                    ...
                    sorted_by_time_json = sorted(json_result, key=LinkParametersHolder.__get_time)
                    if __debug__:
                        print("UPD JSON WITH TIME:", sorted_by_time_json)
